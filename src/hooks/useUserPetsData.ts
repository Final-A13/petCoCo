 ...
 * 
 * @returns userPets - 사용자의 반려동물에 대한 정보를 포함하는 배열, 타입 : UsersPetType[]
 * @returns isPetPending
 * @returns petError
 *  
 * * 현재 로그인한 사용자의 userId를 사용하여 해당 사용자의 반려동물 정보를 가져오는 훅
 * * 가져온 데이터는 배열 형태로, 반려 동물이 한 마리가 아니라 여러 마리일 수도 있기 때문임.
